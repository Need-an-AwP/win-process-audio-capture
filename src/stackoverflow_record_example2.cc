...
public:
    STDMETHOD(ActivateCompleted)(IActivateAudioInterfaceAsyncOperation *operation)
    {
        ...
        hr = operation->GetActivateResult(&hr, &punkAudioInterface);
        if(SUCCEEDED(hr)){
            std::cout << "activate result hr Hexadecimal value: " << std::hex << std::showbase << hr << std::endl;
        }
        return S_OK;
    }
};
...
int main()
{
    ...
    AudioInterfaceCompletionHandler handler;
